 ...
 *   - Action logs
 *
 * All event handling uses document-level delegation so it survives
 * dj:navigate body replacements.
 */
...
export function initDebug() {
  ...
  if (_initialized) {
    // After navigate — just re-create FAB if missing
    _ensureFab();
    return;
  }
  _initialized = true;
...
// ── Delegated events (once, survives navigate) ──────────────────────────────
...
function _bindDelegatedEvents() {
  ...
  document.addEventListener("click", (e) => {
    const fab = e.target.closest("#djact-debug-fab");
    if (!fab) return;
  ...
  // Panel close button
  document.addEventListener("click", (e) => {
    if (e.target.closest("#djd-close-btn")) {
      _closePanel();
    }
  });
  ...
  // Panel tab clicks
  document.addEventListener("click", (e) => {
    const tab = e.target.closest(".djd-tab");
    if (!tab) return;
    _activeTab = tab.dataset.djdTab;
    _renderPanel();
  });
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (Math.abs(dx) > 3 || Math.abs(dy) > 3) dragTarget.dataset.dragged = "1";
...
function _ensureFab() {
  if (document.getElementById("djact-debug-fab")) return;
...
function _updateBadge() {
  ...
  if (!fab) return;
  let badge = fab.querySelector(".djd-badge");
...
function _openPanel() {
  _panelOpen = true;
  let panel = document.getElementById("djact-debug-panel");
  if (!panel) {
    panel = document.createElement("div");
    panel.id = "djact-debug-panel";
...
function _renderPanel() {
  ...
  const body = panel.querySelector("#djd-body");
  if (_activeTab === "requests") _renderRequests(body);
  else if (_activeTab === "errors") _renderErrors(body);
  else if (_activeTab === "logs") _renderLogs(body);
}
...
function _renderRequests(el) {
  if (_logs.length === 0) {
    el.innerHTML = `<div class="djd-empty">No requests yet. Interact with a component.</div>`;
    return;
  }

  el.innerHTML = _logs.map(log => `
    <div class="djd-card">
      ...
      <details style="margin-top:8px">
        ...
        <div class="djd-json">${_esc(JSON.stringify(log.request, null, 2))}</div>
      </details>
      <details style="margin-top:6px">
        <summary style="cursor:pointer;color:#64748b;font-size:11px;font-weight:700">RESPONSE DATA</summary>
        <div class="djd-json">${_esc(JSON.stringify(log.response, null, 2))}</div>
      </details>
    </div>
  `).join("");
}
...
function _renderErrors(el) {
  if (_errors.length === 0) {
    el.innerHTML = `<div class="djd-empty">No errors. Everything is working. ✓</div>`;
    return;
  }

  el.innerHTML = _errors.map(err => `
    <div class="djd-error-item">
      <div class="djd-error-type">${err.type} error</div>
      <div class="djd-error-msg">${_esc(err.message)}</div>
      <div class="djd-error-source">${_esc(err.source || "")} · ${err.time}</div>
    </div>
  `).join("");
}
...
function _renderLogs(el) {
  ...
  if (all.length === 0) {
    el.innerHTML = `<div class="djd-empty">No activity yet.</div>`;
    return;
  }
  ...
  el.innerHTML = all.map(l => `
    <div class="djd-log-item">
      ...
      <span class="djd-log-action">${_esc(l.action)}</span>
    </div>
  `).join("");
}
...
function _injectStyles() {
  ...
  const css = `
    ...
    .djd-header h3 {
      margin: 0; font-size: 14px; font-weight: 800; color: #38bdf8;
      display: flex; align-items: center; gap: 6px;
    }
    .djd-close {
      background: none; border: none; color: #64748b; font-size: 18px;
      cursor: pointer; padding: 4px; border-radius: 6px; transition: all 0.1s;
    }
    ...
    .djd-body {
      flex: 1; overflow-y: auto; padding: 12px 16px; max-height: 50vh;
    }
    ...
    .djd-json {
      ...
      font-size: 11px; color: #94a3b8; overflow-x: auto; max-height: 200px;
      overflow-y: auto; white-space: pre-wrap; word-break: break-all; margin-top: 6px;
    }
    ...
    .djd-error-item {
      background: #1c1017; border: 1px solid #7f1d1d;
      border-radius: 8px; padding: 10px; margin-bottom: 8px;
    }
    .djd-error-type { font-size: 10px; font-weight: 800; color: #ef4444; text-transform: uppercase; margin-bottom: 4px; }
    .djd-error-msg { color: #fca5a5; font-size: 12px; word-break: break-word; }
    .djd-error-source { color: #6b7280; font-size: 10px; margin-top: 4px; }

    .djd-log-item {
      padding: 6px 0; border-bottom: 1px solid #1e293b;
      display: flex; gap: 8px; align-items: flex-start;
    }
    .djd-log-time { color: #475569; font-size: 10px; font-family: monospace; white-space: nowrap; min-width: 72px; }
    .djd-log-action { color: #e2e8f0; font-size: 12px; word-break: break-word; }

    .djd-status { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 700; }
